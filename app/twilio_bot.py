...
class TwilioTextBot:
    ...
    def update_ip_rate(self, ip):
        "If this IP has tried to send more than `limit` messages in that last n_minutes, raise ratelimit"
    ...
    def send_message(self, to_number, message_body, from_ip = None):
        ...
        message_body = message_body[:1600] + "\n\nThis message was sent by the RemindMe bot. Reply STOP to unsubscribe."
    ...
    def check_message_history(self, to_number, n_minutes = 60, limit = 4):
        ...
        filtered_timestamps = [timestamp for timestamp in timestamps if current_time_utc - timestamp <= timedelta(minutes=n_minutes)]
        if len(filtered_timestamps) >= limit:
            raise RateLimitError(None, message = 'Too many messages sent to this number in the last hour')

    def _update_message_history(self, to_number):
        current_time_utc = datetime.utcnow()
        self.to_number_history[to_number] = self.to_number_history.get(to_number, []) + [current_time_utc]